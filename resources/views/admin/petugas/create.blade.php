...
@section('page-description', 'Berikan hak akses ke sistem untuk kolega baru')
...
@section('content')
    <div class="top-toolbar">
        <a href="{{ route('admin.petugas.index') }}" class="btn-back">
            <i class="fas fa-arrow-left"></i> Kembali ke Daftar Staf
        </a>
    </div>
    ...
    <div class="premium-form-container">
        <div class="form-header">
            ...
            <div class="header-text">
                <h3>Buat Akun Tim</h3>
                <p>Silahkan isi data diri anggota tim yang akan didaftarkan</p>
            </div>
        </div>
        ...
        <form action="{{ route('admin.petugas.store') }}" method="POST">
            ...
            <div class="form-body">
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        <input type="text" id="name" name="name" class="premium-input @error('name') is-invalid @enderror" value="{{ old('name') }}" placeholder="Contoh: Budi Santoso" required>
                        @error('name') <span class="invalid-feedback">{{ $message }}</span> @enderror
                    </div>

                    <div class="form-group">
                        <label class="form-label" for="email">Alamat Email <span class="required">*</span></label>
                        <input type="email" id="email" name="email" class="premium-input @error('email') is-invalid @enderror" value="{{ old('email') }}" placeholder="budi@example.com" required>
                        @error('email') <span class="invalid-feedback">{{ $message }}</span> @enderror
                    </div>

                    <div class="form-group">
                        <label class="form-label" for="phone">Nomor Tlp / WhatsApp</label>
                        <input type="text" id="phone" name="phone" class="premium-input @error('phone') is-invalid @enderror" value="{{ old('phone') }}" placeholder="08123456789">
                        @error('phone') <span class="invalid-feedback">{{ $message }}</span> @enderror
                    </div>

                    <div class="form-group">
                        <label class="form-label" for="password">Kata Sandi (Password) <span class="required">*</span></label>
                        <input type="password" id="password" name="password" class="premium-input @error('password') is-invalid @enderror" placeholder="Min. 4 karakter" required minlength="4">
                    ...
                    <div class="form-group full">
                        ...
                        <div class="role-cards">
                            <label class="role-card role-admin">
                                <input type="radio" name="role" value="admin" class="role-input" {{ old('role') == 'admin' ? 'checked' : '' }} required>
                                <div class="role-card-content">
                                    <div class="role-icon"><i class="fas fa-shield-alt"></i></div>
                                    <div class="role-title">Admin Utama</div>
                                    <div class="role-desc">Akses penuh ke seluruh sistem. Termasuk mengelola produk, pengguna, dan pengaturan sistem lainnya.</div>
                                </div>
                            </label>

                            <label class="role-card role-petugas">
                                <input type="radio" name="role" value="petugas" class="role-input" {{ old('role', 'petugas') == 'petugas' ? 'checked' : '' }} required>
                                <div class="role-card-content">
                                    <div class="role-icon"><i class="fas fa-user-check"></i></div>
                                    <div class="role-title">Petugas</div>
                                    <div class="role-desc">Akses terbatas. Umumnya hanya dapat melihat/memproses pesanan dan tidak dapat mengubah pengaturan toko.</div>
                                </div>
                            </label>
                        </div>
            ...
            <div class="form-footer">
                <button type="submit" class="btn-submit">
                    Simpan Akun Baru <i class="fas fa-save"></i>
                </button>
            </div>
        </form>
    </div>
